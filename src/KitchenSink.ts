...
export const KitchenSink = element('kitchen-sink')(
  class extends Element {
    ...
    #awesomenessHandler: EventListener | null = null;

    // CONTINUE add a non-decorator helper in place of @event
    get onawesomeness() {
      return this.#awesomenessHandler;
    }
    set onawesomeness(handler) {
      if (this.#awesomenessHandler) this.removeEventListener('awesomeness', this.#awesomenessHandler);
      this.#awesomenessHandler = handler;
      if (handler) this.addEventListener('awesomeness', handler);
    }

    constructor() {
      super();

      signalify(this, 'count', 'name', 'color', 'doingSomething');
    }
